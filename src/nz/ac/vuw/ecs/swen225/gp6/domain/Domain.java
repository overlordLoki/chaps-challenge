...
public class Domain {
  ...
  public Domain(List<Level> levels, int currentLvl) {
    if (levels == null) {
      throw new NullPointerException("list of levels cannot be null.");
    }
    if (currentLvl < 1 || currentLvl > levels.size()) {
      throw new IndexOutOfBoundsException(
          "currentLvl must be greater than 0(Domain), and smaller or equal to levels.size()");
    }
  ...
  /**
   * constructor with a list of mazes, and an inventory for current level (all other levels get an
  ...
  public List<Maze> getMazes() {
    return levels.stream().map(l -> l.maze).toList();
  }
  ...
  public Maze getCurrentMaze() {
    return getCurrentLevelObject().maze;
  }
  ...
  public Inventory getInv() {
    return getCurrentLevelObject().inv;
  }
  ...
  public Level getCurrentLevelObject() {
    return levels.get(currentLvlIndex - 1);
  }

  /**
   * returns the list of level time limits, where each element corresponds to a level, and the level
   * is restarted if time runs out.
   *
   * @return the list of level time limits (in order)
   */
  public List<Integer> getLevelTimeLimits() {
    return levels.stream().map(l -> l.timeLimit).toList();
  }
   ...
   */
  public void moveUp() {
    getCurrentLevelObject().makeHeroStep(Direction.Up);
  }
  ...
  public void moveDown() {
    getCurrentLevelObject().makeHeroStep(Direction.Down);
  }
  ...
  public void moveLeft() {
    getCurrentLevelObject().makeHeroStep(Direction.Left);
  }
  ...
  public void moveRight() {
    getCurrentLevelObject().makeHeroStep(Direction.Right);
  }
   ...
   * @throws IndexOutOfBoundsException if lvl is not at least 1 or is bigger than the number of
   *                                   levels
   */
  ...
  public int getCurrentTimeLimit() {
    return getLevelTimeLimits().get(currentLvlIndex - 1);
  }
  ...
  public long getCurrentTime() {
    return levels.get(currentLvlIndex - 1).getCurrentTime();
  }
  ...
  public void setCurrentTime(long time) {
    getCurrentLevelObject().setCurrentTime(time);
  }
  ...
  public int getTreasuresLeft() {
    return getCurrentMaze().getTileCount(TileType.Coin);
  }
  ...
  public boolean isLastLevel() {
    return currentLvlIndex == levels.size();
  }
  ...
  public boolean heroIsOnInfo() {
    return ((Hero) (getCurrentMaze().getTileThat(t -> t.type() == TileType.Hero))).tileOn().type()
        == TileType.Info;
  }

  /**
   * gets the message stored in the info tile of this level (note every level can have one info tile
   * at max), must ONLY BE CALLED when hero is on tile info.
   *
  ...
  public String getInfoHint() {
    if (!heroIsOnInfo()) {
      throw new RuntimeException("hero is not on info");
    }
    return ((Info) (((Hero) getCurrentMaze().getTileThat(
        t -> t.type() == TileType.Hero)).tileOn())).message();
  }
  ...
  public Tile[][] getGameArray() {
    return getCurrentMaze().getTileArrayCopy();
  }
  ...
  public void addEventListener(DomainEvent event, Runnable toRun) {
    if (event == null || toRun == null) {
      throw new NullPointerException(
          "DomainEvent or Runnable associated cannot be null (Domain.addEventListener)");
    }
