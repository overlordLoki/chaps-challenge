...
public class Level {
   ...
   */
  public Level(Maze maze, Inventory inv, int lvl, int timeLimit, long timeCurrent,
      Direction heroNextStep) {
    //pre production checks
    if (timeLimit < 0 || timeCurrent < 0) {
      throw new IndexOutOfBoundsException("time limit/current cannot be < 0 (Level)");
    }
    if (inv == null || maze == null || heroNextStep == null) {
      throw new NullPointerException("inv, maze or heroNextStep cannot be null (Level)");
    }
    if (lvl < 1) {
      throw new IndexOutOfBoundsException("lvl cannot be < 1 (Level)");
    }
  ...
  /**
   * constructor for a level, which takes the maze, and level number. It default the time limit to
  ...
  public void setCurrentTime(long timeCurrent) {
    if (timeCurrent < 0) {
      throw new IllegalArgumentException(
          "timeCurrent cannot be negative. (Level.setCurrentTime())");
    }
