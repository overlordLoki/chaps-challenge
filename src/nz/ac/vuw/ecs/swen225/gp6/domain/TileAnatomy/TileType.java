...
/**
 * Each tile object will have a type field, which helps to determine what the tile is. Note1: Any
...
public enum TileType {
  ...
  /**
   * this is to make the associated tile class instance for the given tile type. NOTE: The enum name
   * and Tile class name must be the same.
   *
   * @param t    tile type to make
   * @param info an instance of the class TileInfo that has all the necessary information that tile
   *             needs
   * @return an instance of the desired tile class
   * @throws IllegalArgumentException if a file in tiles with same name as tile type given is not
   *                                  found
   * @throws NullPointerException     if t or info are null
   */
  public static Tile makeTile(TileType t, TileInfo info) {
    if (t == null || info == null) {
      throw new NullPointerException("tile type or tile info cannot be null (TileType.makeTile)");
    }
    try {
   ...
   * @param info   an instance of the TileInfo object that has all the necessary initial information
   *               that tile needs
   * @return an instance of the desired tile class
  ...
  public static Tile makeTileFromSymbol(char symbol, TileInfo info) {
    ...
    TileType type = Arrays.stream(TileType.values())
        .filter(t -> {
          try {
            return TileType.makeTile(t, new TileInfo(null)).symbol() == symbol;
          } catch (Exception e) {
            return false;
          }
        }).
